...
import KYCUserCardInfo from './containers/KYCUserCardInfo';
import KYCBlockpassInfo from './containers/KYCBlockpassInfo';
import KYCCertificateInfo from './containers/KYCCertificateInfo';
import { LeftSideBar, RightSideBar } from './containers/Sidebar';
 ...
 */
	...
	_reviewCertificateEvt = (rawData) => {
		...
		Modal.info({
			...
			width: 750,
			content: <pre style={codeDialogStyle}>{JSON.stringify(rawData, null, 2)}</pre>,
		...
		if (checkResubmit) {
			...
			if (!reSubmitData.length) {
				...
				reSubmitData = reSubmitData
					.filter((item) => changeLogs.some((submitData) => submitData.slug === item.keyName))
					.map((item) => ({
						...item,
 ...
 */
	...
	async approveCertificate() {
		...
		if (
			reviewProcess &&
			path([ 'body', 'cer', 'data', 'bpSignRes', 'res', 'status' ])(reviewProcess) === 'success'
		) {
			this.setState({ modal: '', submitReason: '' });
			message.success('A certificate has been issued!');
		...
		log
			.map((item) => {
				if (item.message === 'field-decision' && path([ 'extra', 'slug' ])(item) === category) {
					...
					const checkFieldReSubmitted = path([ 'extra', 'changeLogs' ])(item).some(
 ...
 */
	...
	_updateFieldStatus = (fieldData, latestStatus, profileStatus) => {
		...
		const fieldStatus = fieldHistory.length > 0 ? fieldHistory[0].status : 'submitted';
		const fieldComment = fieldHistory.length > 0 ? fieldHistory[0].comment : '';
		...
		return {
			...fieldData,
			status: latestStatus === 'received' && lastSubmitData ? 'submitted' : fieldStatus,
			reason: latestStatus === 'received' && lastSubmitData ? '' : fieldComment
		};
	};
 ...
 */
	...
	_getLatestSubmittedData = ({ lastSubmitData, category, reviews, type, logStory, isWaitingUserResubmit }) => {
		...
		if (changeLog) {
			...
			if (changeLog && changeLog.new !== changeLog.old) {
				return {
					...
					value: type === 'image' ? translatePictureUrl(changeLog.old) : changeLog.old
				};
			} else return null;
		} else return null;
	};
 ...
 */
	_checkLogData = ({ log, reviews, category, type, isWaitingUserResubmit }) => {
		if (log.length === 0) return { currentSubmitted: null, logStory: [], lastSubmitData: null };
		...
		const lastSubmitData = log.find(
			(item) => item.message === 'record-waiting' && path([ 'extra', 'submitCount' ])(item) === currentSubmitted
		);
			...
			this.setState(
				...
				() =>
					renderModal({
						...
						history: type === 'history' ? this._getLogFieldData(this.state.profileData)(title) : ''
					})
			);
		}
	};
 ...
 */
	_zoomEvt = (params) =>
		renderModal({
			...
			waitingUserResubmit: this.state.waitingUserResubmit,
			rejectedEvt: (keyName, reason) =>
				this.setState({ submitReason: reason, infoItemChecking: keyName }, () =>
					this._onSubmitHandle('rejected')
				),
			acceptedEvt: (keyName) => this._showModal(keyName, 'approved')
		});
 ...
 */
	_nextEvt = () =>
		renderModal({
			modalTitle: 'REVIEW CERTIFICATE',
			type: 'confirm',
			value: this.state.submitReason,
			onSubmit: this._onSubmitHandle,
			backEvt: this._backEvt
		});
 ...
 *
 */
	_getLogFieldData = (data = this.state.profileData) => (category = this.state.infoItemChecking) =>
		data && category && path([ category, 'history', 'logStory' ])(data);
	...
	render() {
		...
		const profileCheckSideBarData = keys(profileData).map((item) => ({
			title: item,
			status:
				profileData[item].status === 'approved' || profileData[item].status === 'rejected'
			...
			status === 'waiting' || waitingUserResubmit
				...
				: !(
						profileCheckSideBarData.every(
							(item) => item.status === 'rejected' || item.status === 'approved'
		...
		return (
			...
			profileData && (
				<Row key="kyc-main">
					...
					<Col span={collapsed ? 14 : 18} className="kyc-main">
						<div className="kyc-detail">
							<KYCUserCardInfo
								...
								hasCerts={Object.keys(this.state.certs || {}).length > 0}
							/>{' '}
							{keys(this.state.profileData).length > 0 && (
								<KYCBlockpassInfo
									...
									status={this.state.waitingUserResubmit ? 'waiting' : status}
									waitingUserResubmit={waitingUserResubmit}
									zoomEvt={this._zoomEvt}
							...
							<div
								...
								style={{
									display: status === 'approved' ? 'none' : 'flex',
									maxWidth: '800px',
									margin: '0 auto',
									padding: '0 95px'
								}}
							>
								{status !== 'approved' && (
									<Fragment>
										<Button
											className={`big blue ${feedBackDisableStatus ? 'disabled' : ''}`}
										...
										<Button
											className={`big ${approvalDisableStatus ? 'disabled' : ''}`}
					...
					<Col span={this.props.collapsed ? 6 : 0} className="kyc-right-side">
						{
							<RightSideBar
