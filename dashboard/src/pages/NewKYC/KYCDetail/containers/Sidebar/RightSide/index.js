...
class RightSideBar extends Component {
	...
	render() {
		...
		return (
			<div className={`right-side ${this.props.open ? 'open' : 'hide'}`}>
				<h4 className="right-side__title">LATEST ACTIVITIES</h4>
				<div className="history-component">
					{historyData.length > 0 ? (
						historyData.map((item, index) => (
							<div
								className="history-component__item"
								key={`hist-${index}`}
							>
								...
								<div className="history-component__item-info">
									<p className="history-component__item-detail">
										<span
											className={`history-component__item-status ${this.messageFormat(
												item
											).status.toLowerCase()}`}
										>
											{this.messageFormat(item).status}
										</span>
										<span>
											{MAP_KEYWORDS[
												this.messageFormat(item).msg
											] || this.messageFormat(item).msg}
										</span>
									</p>
