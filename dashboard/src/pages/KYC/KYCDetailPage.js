...
class UserDetailPage extends Component {
    ...
    _handleSubmit = (values) => {
        ...
        Modal.confirm({
            title: 'Are you sure ?',
            content: 'We will notify user that. Their review process is starting.',
            onOk: async () => {
                const startReviewProcess = await ApplicationStore.startReview(_id, 'beginning review');
                if (!startReviewProcess) {
                    console.error(startReviewProcess.body);
                    return this._showError('server error');
                }

                return Promise.all([this._fetchData(), this._fetchHistory(), this._fetchCertificate()]);
            },
            onCancel() {
    ...
    _handleReview = (values) => {
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                if (isAccept)
                    reviewProcess = await ApplicationStore.approveCertificate(_id, comments, others);
                else
                    reviewProcess = await ApplicationStore.rejectCertificate(_id, comments, others);
    ...
    render() {
        ...
        return <div>
            <Steps current={step}>
                <Step title="Waiting Info" description="Waiting user input enough infomation" />
                <Step title="In Review" description="KYC Procedure" />
                <Step title="Complete" description="Issue Certified" />
            </Steps>

            <Card title="User Information" bordered={false} style={{ paddingTop: 20 }}>
                <Row type="flex" justify="space-around">
                    ...
                    }
                    {
                        step !== 2 && history &&
                        <Col span={6} >
                            <Card title="History" bordered={false} style={{ height: 400 }}>
            ...
            {
                step === 1 && <Card title="Reviewer Input" bordered={false} style={{ paddingTop: 20 }}>
                    <ReviewSections 
            ...
            {
                step === 2 && <Card title="History" bordered={false} style={{ paddingTop: 20 }}>
                    <UserActivityHistory history={history} />
                </Card>
            }
