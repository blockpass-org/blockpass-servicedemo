...
export default class ApplicationStore {
    ...
    @action async login(userName, pass) {
        this.showLoading();
    ...
    @action async restQueryData(model, queryObj = {}) {
        if (!this._auth.loggedIn())
            return null;

        try {
            let normalizeObj = {}
            Object.keys(queryObj).forEach(key => normalizeObj[key] = JSON.stringify(queryObj[key]))
            let response = await request
    ...
    @action async restUpdateData(model, modelData = {}) {
        if (!this._auth.loggedIn())
            return null;
    ...
    @action async restCreateData(model, modelData = {}) {
        if (!this._auth.loggedIn())
            return null;
    ...
    @action async startReview(id, message) {
        if (!this._auth.loggedIn())
            return null;
    ...
    @action async approveCertificate(id, message, others) {
        if (!this._auth.loggedIn())
            return null;
    ...
    @action async rejectCertificate(id, message) {
        if (!this._auth.loggedIn())
            return null;
    ...
    @action async getProfileSetting(id) {
        if (!this._auth.loggedIn())
            return null;
    ...
    @action async updatePassword(pass) {
        if (!this._auth.loggedIn())
            return null;
    ...
    @action async updateSetting(data) {
        if (!this._auth.loggedIn())
            return null;
        ...
        try {
            ...
            if(response && response.body)
                this.appSetting = [
                    ...this.appSetting.filter(setting => setting.label !== response.body.label),
                    response.body
                ];
    ...
    @action async querySetting() {
        try {
            this.showLoading()

            let response = await request
                .get(api.appSetting(this.hostUrl))
            
            if(response && response.body)
                this.appSetting = response.body;
    ...
    @action async firstTimeSetup(payload) {
        try {
            let response = await request
