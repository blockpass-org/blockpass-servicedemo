...
class Auth {
    ...
    @action setToken = (_token, _expire) => {
        // console.log('Auth::setToken', _token, _expire);
        this.token = _token || '';
        this.expire = _expire || 0;
    }

    @action setPermission = (_permissions) => {
        try {
            if (Array.isArray(_permissions))
                this.scope = _permissions;
        } catch (err) {
            this.scope = [];
        }
    }

    @action checkTokenExpire = (response) => {
    ...
    hasOneOfPermission(_permissionList) {
        return _permissionList.some(value => this.scope.indexOf(value) !== -1)
    }
    ...
    saveState = () => {
        ...
        window.localStorage.setItem(constants.AUTH_KEY_NAME, JSON.stringify({ token: this.token, expire: this.expire, scope: this.scope }))
    }
