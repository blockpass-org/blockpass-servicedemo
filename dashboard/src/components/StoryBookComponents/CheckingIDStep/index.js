...
class UserCard extends Component {
		static propTypes = {
				...
				userData: PropTypes.shape({match: PropTypes.number, unmatch: PropTypes.number}),
				/**  action to checkID */
		...
		render() {
				...
				return (
						<div
								...
								? ''
								: 'full-width'}`}>
								<Row className="user-card__wrapper" style={{display: 'flex', flexDirection: `${collapsed ? 'column': 'row'}` }}>
										<Col
												span={!collapsed ? status === 'waiting'
												? 16
												: 14 : 24}
												className={cardWrapperClass}>
												<div className={`info ${status}`}>
														{(status === 'approved' || status === 'rejected') && <div className={`info__status`}>
																<span className={`info__status-value ${status}`}>{status.toUpperCase()}</span>
																<span className="info__status-date">09 Jun 2018</span>
														</div>}
														...
														<div className="info__item">
																...
																<span className="info__value">{userEmail}</span>
														</div>
												</div>
										</Col>
										{status === 'waiting' && (
												<Col
														...
														? 'result'
														: ''}`}
														style={{ margin: `${collapsed ? '15px auto 0': ''}` }}
														>
														<Button className="medium" onClick={this.checkIDhandler}>
																START REVIEW
														</Button>
												</Col>
										)}
